...
function playerFilter(item, other)
  if other.block then return "touch"
  elseif other.type then return "cross"
  end
end
...
local function updatePlayer(dt)
  ...
  if dx ~= 0 or dy ~= 0 then
    ...
    for i=1, cols_len do
      local col = cols[i]
      consolePrint(("col.item = %s, col.other = %s, col.type = %s, col.normal = %d,%d"):format(col.item,col.other, col.type, col.normal.x, col.normal.y))
    end
  end
end
